...
async function fix() {
  ...
  } catch (e: any) { console.warn("Could not drop Silver.embedding (might not exist)"); }

  try {
    await prisma.$executeRawUnsafe(`ALTER TABLE "SilverProductMention" DROP COLUMN "embedding768";`);
    console.log("Dropped Silver.embedding768");
  } catch (e: any) { console.warn("Could not drop Silver.embedding768 (might not exist)"); }

  try {
    await prisma.$executeRawUnsafe(`ALTER TABLE "GoldProduct" DROP COLUMN "embedding";`);
    console.log("Dropped Gold.embedding");
  } catch (e: any) { console.warn("Could not drop Gold.embedding (might not exist)"); }

  try {
    await prisma.$executeRawUnsafe(`ALTER TABLE "GoldProduct" DROP COLUMN "embedding768";`);
    console.log("Dropped Gold.embedding768");
  } catch (e: any) { console.warn("Could not drop Gold.embedding768 (might not exist)"); }
