...
const AssetNode = ({ data }: any) => {
  ...
  const getLayerColor = (layer: string) => {
    switch (layer) {
      case "bronze": return "border-amber-700/50 bg-amber-950/20 text-amber-500";
      case "silver": return "border-slate-500/50 bg-slate-900/40 text-slate-300";
      case "gold": return "border-yellow-500/50 bg-yellow-950/20 text-yellow-500";
      default: return "border-white/10 bg-black text-white";
    }
  };
    ...
    >
      ...
      <div>
        ...
        <div className="flex gap-2 mt-3 text-xs text-muted-foreground font-mono">
          <span className="bg-background/50 px-2 py-0.5 rounded flex items-center gap-1"><Zap className="w-3 h-3"/> {data.rules} QA Rules</span>
          {data.isPartitioned && <span className="bg-background/50 px-2 py-0.5 rounded">Partitioned</span>}
        </div>
      </div>
  ...
  return (
    <div className="w-full h-[calc(100vh-100px)] rounded-xl border overflow-hidden bg-background relative shadow-inner">
      <div className="absolute top-4 left-4 z-10 bg-card border px-4 py-2 rounded-lg shadow-lg">
        ...
        <p className="text-xs text-muted-foreground">Medallion architecture auto-discovered from registry.ts</p>
      </div>
