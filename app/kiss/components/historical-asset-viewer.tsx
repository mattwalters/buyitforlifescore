...
export function HistoricalAssetViewer({ assetId }: { assetId: string }) {
  ...
  const loadData = async () => {
    setLoading(true);
    const [matRes, jobRes] = await Promise.all([getAssetHistory(assetId), getJobs(assetId)]);
      ...
      <div className="flex justify-between items-center border-b pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Persisted execution states tracking QA metrics across partitions.
          </p>
        </div>
        <button
          onClick={handleRun}
        ...
        <>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="border bg-card rounded-xl overflow-hidden shadow-sm">
              <div className="p-4 bg-muted/40 border-b">
                <h3 className="font-semibold text-sm flex items-center gap-2 font-mono">
                  <Clock className="w-4 h-4" /> Execution Ledger
                </h3>
              </div>
              <ul className="divide-y max-h-96 overflow-y-auto">
                ...
                {jobs.map((job) => (
                  <li
                    key={job.id}
                    className="p-4 text-sm flex items-center justify-between hover:bg-muted/30"
                  >
                    <div>
                      ...
                      {job.errorTrace && (
                        <p className="text-xs text-destructive mt-1 font-mono truncate max-w-xs">
                          {job.errorTrace}
                        </p>
                      )}
                    </div>
            ...
            <div className="border bg-card rounded-xl overflow-hidden shadow-sm">
              <div className="p-4 bg-muted/40 border-b">
                <h3 className="font-semibold text-sm flex items-center gap-2 font-mono">
                  <CheckCircle2 className="w-4 h-4" /> Validated Materializations
                </h3>
              </div>
              <ul className="divide-y max-h-96 overflow-y-auto">
                {materializations.length === 0 && (
                  <li className="p-4 text-sm text-muted-foreground text-center">
                    No materializations stored.
                  </li>
                )}
                {materializations.map((m) => {
                  ...
                  return (
                    <li
                      key={m.id}
                      className="p-4 text-sm flex flex-col gap-2 hover:bg-muted/30 cursor-pointer"
                    >
                      <div className="flex justify-between">
                        <span className="font-semibold font-mono text-primary">
                          {m.partitionKey || "Global"}
                        </span>
                        <span className="text-muted-foreground">
                          {Number(m.summaryPayload?.totalRows || 0).toLocaleString()} rows
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {qaResults.length === 0 && (
                          <span className="text-xs text-slate-500">No QA Rules</span>
                        )}
                        {qaResults.map((qa, i) => (
                          <span
                            key={i}
                            className={`text-xs px-1.5 py-0.5 rounded flex gap-1 items-center ${qa.passed ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500 font-bold"}`}
                          >
                            {qa.passed ? (
