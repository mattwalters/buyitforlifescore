...
def main():
    ...
    asset_name = questionary.select(
        "Which partitioned asset do you want to safely sample?", choices=dynamic_assets
    ).ask()
    ...
    upstream = questionary.confirm(
        "Resolve missing upstream dependencies lazily? (Dagster will auto-fill what it needs)", default=True
    ).ask()
    ...
    """
    try:
        res = requests.post(gql_url, json={"query": materialized_query, "variables": {"assetKey": asset_name}}).json()
    ...
    partitions_by_assets = [{"assetKey": {"path": [a]}, "partitions": {"ranges": ranges}} for a in target_assets]
    ...
    variables = {"backfillParams": {"partitionsByAssets": partitions_by_assets}}
