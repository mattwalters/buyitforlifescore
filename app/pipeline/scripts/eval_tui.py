...
def get_model():
    ...
    if choice == "6":
        return input("Enter custom model string: ").strip()
...
def get_thinking(model):
    if "gemini-3" in model:
        ...
        if choice == "3":
            return "4096"
        if choice == "4":
            return input("Enter token limit number (integer): ").strip()
...
def main():
    ...
    verbose = (
        get_choice("\nPrint detailed mismatch logs? (y/n) (Default y): ", ["y", "n", "Y", "N"], default="y").lower()
        == "y"
    )
