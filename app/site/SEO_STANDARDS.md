...
*This document serves as the absolute baseline architectural standard for the frontend. Any deviation from these rules guarantees a degradation in E-E-A-T trust, crawlability, or AI ingestion. Our target is a permanent 100/100 Lighthouse score.*
...
*   **Rule:** Every visual data chart *must* be immediately followed by a `<table class="sr-only">`.
...
*Every single page must inject a highly targeted `application/ld+json` script into the `<head>`.*
