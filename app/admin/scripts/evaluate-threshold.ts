...
async function main() {
  ...
  for (const b of popularBrands) {
    const mentions = await prisma.$queryRaw<
      Array<{ brand: string; productName: string; emb7: string }>
    >`
  ...
  console.log("=======================================================");
  const highTier = shuffleArray(pairs.filter((p) => p.sim >= 0.9)).slice(0, 10);
  highTier
    .sort((a, b) => b.sim - a.sim)
    .forEach((p) => console.log(`[Sim: ${p.sim.toFixed(3)}] ${p.str1}  <==>  ${p.str2}`));
