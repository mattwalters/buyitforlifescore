...
const llmResponseSchema: Schema = {
  ...
  properties: {
    departmentIndex: {
      type: Type.INTEGER,
      description: "The 1-based index (e.g. 1, 2, 3) of exactly ONE department from the list.",
    },
    categoryIndices: {
      type: Type.ARRAY,
      items: { type: Type.INTEGER },
      description:
        "An array of 1-based indices representing the specific sub-categories this product applies to (choose 1 to 6).",
    },
  },
...
async function main() {
  ...
  const processLine = async (line: (typeof lines)[0], i: number) => {
    ...
    const threadTitles = Array.from(
      new Set(line.mentions.map((m) => m.submission?.title).filter(Boolean)),
    );

    // Format them for the prompt
    const ideasContext = ideas.length > 0 ? ideas.join(", ") : "None";
    const titleContext =
      threadTitles.length > 0 ? threadTitles.map((t) => `- "${t}"`).join("\n") : "None";
    ...
    if (vectorLiteral) {
      ...
      topCategories = await prisma.goldCategory.findMany({
...
3. Select an array of index numbers from the Candidate Categories list that this product acts as a member of.`;
    ...
    try {
      ...
      if (response.text) {
        const result: { departmentIndex: number; categoryIndices: number[] } = JSON.parse(
          response.text,
        );
        ...
        if (result.departmentIndex && result.categoryIndices.length > 0) {
          ...
          if (validDept && validCats.length > 0) {
            ...
            console.log(
              `   [Routing] 🎯 Routed to Department: '${validDept.canonicalName}' and ${validCats.length} Categories.`,
            );
          } else {
            console.log(
              `   [Routing] ⚠️ LLM hallucinated indices. ${result.departmentIndex} and [${result.categoryIndices.join(",")}] are out of bounds. Skipping.`,
            );
          }
        }
