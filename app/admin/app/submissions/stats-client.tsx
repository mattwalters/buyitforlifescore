...
export type TrackerDatum = { chars: number; mentions: number; score: number; comments: number; isProcessed: boolean };
...
export default function TokenStatsChart({ 
  ...
  const commentHistogramData = useMemo(() => {
    ...
    const maxComments = commentValues.length > 0 ? commentValues.reduce((a, b) => Math.max(a, b), 0) : 0;
  ...
  const avgChars = totalProcessed > 0 ? Math.round(data.map(d => d.chars).reduce((a,b) => a+b, 0) / totalProcessed) : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm xl:col-span-2">
          <h3 className="font-semibold mb-2">Signal-to-Noise: Thread Size vs Yield (Processed Only)</h3>
          <p className="text-sm text-muted-foreground mb-6">Observe if longer threads map linearly to higher product yields. Limited to threads that have successfully cleared the AI Extraction Pipeline.</p>
              ...
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart margin={{ top: 20, right: 30, left: 0, bottom: 30 }}>
                  ...
                  <XAxis 
                    ...
                    name="Characters" 
                    tick={{ fill: "#9ca3af" }}
                    label={{ value: "Total Character Length (Window Size)", position: "insideBottom", offset: -25, fill: "#d1d5db", fontSize: 13, fontWeight: "500" }} 
                  />
                  <YAxis 
                    type="number" 
                    dataKey="mentions" 
                    ...
                    tick={{ fill: "#9ca3af" }}
                    label={{ value: "Total Products Extracted", angle: -90, position: "insideLeft", offset: 10, fill: "#d1d5db", fontSize: 13, fontWeight: "500" }}
                  />
                  <ZAxis 
                    type="number" 
                    dataKey="score" 
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          {specificityData.length > 0 ? (
            <div className="h-72 w-full">
             ...
             <div className="h-72 flex items-center justify-center text-muted-foreground text-sm">No specificity data</div>
          )}
        </div>
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          {sentimentData.length > 0 ? (
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height={288}>
                <BarChart data={sentimentData} layout="vertical" margin={{ top: 5, right: 30, left: 30, bottom: 5 }}>
             ...
             <div className="h-72 flex items-center justify-center text-muted-foreground text-sm">No sentiment data</div>
          )}
        </div>
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm xl:col-span-2">
          ...
          {histogramData.length > 0 ? (
              ...
              <ResponsiveContainer width="100%" height={384}>
                <BarChart data={histogramData} margin={{ top: 20, right: 30, left: 20, bottom: 30 }}>
                  ...
                  <XAxis 
                     ...
                     height={80}
                     fontSize={12}
                     label={{ value: "Total Characters", position: "insideBottom", offset: -25, fill: "#d1d5db", fontSize: 14, fontWeight: "500" }}
                  />
                  <YAxis 
                     tick={{ fill: "#9ca3af" }}
                     tickLine={false}
                     ...
                     label={{ value: "Analyzed Threads", angle: -90, position: "insideLeft", offset: -10, fill: "#d1d5db", fontSize: 14, fontWeight: "500" }}
                  />
                  <Tooltip 
                    cursor={{ fill: "hsl(var(--muted)/0.5)" }}
                    contentStyle={{ backgroundColor: "hsl(var(--popover))", borderColor: "hsl(var(--border))", borderRadius: "8px" }}
            ...
            <div className="h-96 w-full flex items-center justify-center text-muted-foreground border-2 border-dashed rounded-lg text-sm px-16 text-center">
               Global Character density is computed at Ingestion, not Extraction. If this space is blank, no threads exist in the Bronze layer. Run ingestion via CLI.
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm xl:col-span-2">
          ...
          {commentHistogramData.length > 0 ? (
            <div className="h-96 w-full">
              <ResponsiveContainer width="100%" height={384}>
                <BarChart data={commentHistogramData} margin={{ top: 20, right: 30, left: 20, bottom: 30 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#374151" />
                  <XAxis 
                     dataKey="range" 
                     tick={{ fill: "#9ca3af" }} 
                     ...
                     height={80}
                     fontSize={12}
                     label={{ value: "Total Comments", position: "insideBottom", offset: -25, fill: "#d1d5db", fontSize: 14, fontWeight: "500" }}
                  />
                  <YAxis 
                     tick={{ fill: "#9ca3af" }}
                     tickLine={false}
