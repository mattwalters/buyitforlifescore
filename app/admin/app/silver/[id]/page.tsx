...
export default async function SilverMentionPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (mention.submissionId) {
    ...
    const extractionSpend = spends
      .filter((s) => s.jobName === "SILVER_EXTRACTION")
      .reduce((sum, s) => sum + s.costInUsd, 0);
    const embeddingSpend = spends
      .filter((s) => s.jobName === "SILVER_EMBEDDING")
      .reduce((sum, s) => sum + s.costInUsd, 0);
  ...
  return (
    <div className="container mx-auto p-8 space-y-8">
      <div className="flex flex-col gap-2">
        <Link href="/silver" className="text-sm text-primary hover:underline">
          &larr; Back to Silver
        </Link>
        <h1 className="text-3xl font-bold tracking-tight">
          Mention: {mention.brand} {mention.productName}
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="rounded-xl border bg-card p-6 shadow-sm">
            ...
            <dl className="grid grid-cols-2 gap-y-4 text-sm">
              ...
              <dd className="font-medium">
                {mention.acquiredPrice ? `$${mention.acquiredPrice}` : "-"}
              </dd>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-6 shadow-sm">
            <h2 className="font-semibold text-primary mb-4 border-b border-primary/10 pb-2">
              AI Cost Profile (Pro-Rata)
            </h2>
            <p className="text-xs text-muted-foreground mb-4">
              This submission yielded {totalMentionsFromSource} mentions. Cost is apportioned
              mathematically.
            </p>
            <dl className="grid grid-cols-2 gap-y-4 text-sm">
              <dt className="text-muted-foreground">Apportioned Extraction</dt>
              <dd className="font-medium font-mono text-green-600">
                ${apportionedExtractionCost.toFixed(6)}
              </dd>

              <dt className="text-muted-foreground">Apportioned Embedding</dt>
              <dd className="font-medium font-mono text-green-600">
                ${apportionedEmbeddingCost.toFixed(6)}
              </dd>

              <dt className="text-muted-foreground font-semibold pt-2 border-t border-primary/10">
                Total Origin Cost
              </dt>
              <dd className="font-bold font-mono text-green-700 pt-2 border-t border-primary/10">
                ${(apportionedExtractionCost + apportionedEmbeddingCost).toFixed(6)}
              </dd>
            </dl>
          </div>
          ...
          <div className="rounded-xl border bg-card p-6 shadow-sm">
            ...
            <dl className="gap-y-4 text-sm space-y-4 flex flex-col">
              <div>
                ...
                {mention.submission ? (
                  <Link
                    href={`/submissions/${mention.submission.id}`}
              ...
              <div>
                ...
                {mention.goldBrand ? (
                  <Link
                    href={`/gold/brands/${mention.goldBrand.id}`}
              ...
              <div>
                ...
                {mention.goldProductLine ? (
                  <Link
                    href={`/gold/lines/${mention.goldProductLine.id}`}
                    className="text-orange-600 hover:underline font-medium"
                  >
              ...
              <div>
                ...
                {mention.goldProduct ? (
                  <Link
                    href={`/gold/models/${mention.goldProduct.id}`}
                    className="text-orange-600 hover:underline font-medium"
                  >
