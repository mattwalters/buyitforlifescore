...
export default function SilverClientTable({
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(
    new Set(["brand", "productName", "specificityLevel", "sentiment", "submissionContext"]),
  );
                    ...
                    visibleColumns.has(col.key) && (
                      <th key={col.key} className="h-10 px-4 align-middle">
                        {col.key !== "submissionContext" && col.key !== "flawOrCaveat" ? (
                          <Link
                            href={`?sortBy=${col.key}&dir=${sortBy === col.key && dir === "asc" ? "desc" : "asc"}`}
                          >
                      ...
                      visibleColumns.has(col.key) && (
                        <td key={col.key} className="p-4 align-middle text-muted-foreground">
                          {col.key === "brand" ? (
                            ...
                            <div
                              className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${getSentimentVariant(mention.sentiment)}`}
                            >
                              {mention.sentiment}
                            </div>
                            ...
                            mention.submission ? (
                              <Link
                                href={`/submissions/${mention.submissionId}`}
