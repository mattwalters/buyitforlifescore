...
export default async function GoldLinePage(props: { params: Promise<{ id: string }> }) {
  ...
  const submissionIds = Array.from(
    new Set(line.mentions.map((m) => m.submissionId).filter(Boolean)),
  ) as string[];
  ...
  if (submissionIds.length > 0) {
    ...
    for (const spend of spends) {
      ...
      const myMentionsInThisSubmission = line.mentions.filter(
  ...
  return (
    <div className="container mx-auto p-8 space-y-8">
      <div className="flex flex-col gap-2">
        <Link href="/gold/lines" className="text-sm text-primary hover:underline">
          &larr; Back to Product Lines
        </Link>
        <div className="flex items-center gap-2">
          ...
          <Link
            href={`/gold/brands/${line.goldBrand.id}`}
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-6 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Represents the apportioned cost to surface {line.mentionCount} child mentions and
