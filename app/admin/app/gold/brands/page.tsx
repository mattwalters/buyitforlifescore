...
import BrandClientTable from "./client-table";
...
export default async function BrandsPage(props: {
  ...
  const brands = await prisma.goldBrand.findMany({
    orderBy: {
      [sortBy]: orderDir,
    },
    take: pageSize,
  ...
  const total = await prisma.goldBrand.count();
  ...
  return (
    <div className="container mx-auto p-8 space-y-8">
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">
            View all {total.toLocaleString()} normalized brands mapped by the AI.
          </p>
        </div>
      ...
      <BrandClientTable brands={brands} sortBy={sortBy} dir={dir} />
