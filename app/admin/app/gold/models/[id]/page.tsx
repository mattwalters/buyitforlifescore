...
export default async function GoldModelPage(props: {
  ...
  const submissionIds = Array.from(new Set(model.mentions.map(m => m.submissionId).filter(Boolean))) as string[];
  ...
  if (submissionIds.length > 0) {
     ...
     for (const spend of spends) {
        ...
        const myMentionsInThisSubmission = model.mentions.filter(m => m.submissionId === spend.submissionId).length;
  ...
  return (
    <div className="container mx-auto p-8 space-y-8">
      <div className="flex flex-col gap-2">
        <Link href="/gold/models" className="text-sm text-primary hover:underline">&larr; Back to Exact Models</Link>
        <div className="flex items-center gap-2">
           ...
           <Link href={`/gold/brands/${model.goldBrand.id}`} className="text-xl text-primary hover:underline font-semibold tracking-tight">
            ...
            <dl className="grid grid-cols-2 gap-y-4 text-sm">
              ...
              {model.goldProductLine ? (
                 <dd className="font-medium">
                   <Link href={`/gold/lines/${model.goldProductLine.id}`} className="hover:underline text-orange-600">
              ...
              <dd className="font-medium">
                 <Link href={`/silver?goldProductId=${model.id}`} className="hover:underline text-primary">
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-orange-500/20 bg-orange-500/5 p-6 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Represents the apportioned cost to surface {model.mentionCount} child mentions and deductually evaluate them into a canonical record.
            </p>
            <dl className="grid grid-cols-2 gap-y-4 text-sm">
