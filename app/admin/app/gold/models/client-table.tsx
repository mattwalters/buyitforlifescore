...
export default function ModelClientTable({ 
  models, 
  sortBy, 
  dir,
}: { 
  ...
  const allColumns = [
    { key: "id", label: "ID" },
    { key: "canonicalName", label: "Model Name" },
                  ...
                  {allColumns.map(col => visibleColumns.has(col.key) && (
                    <td key={col.key} className={`p-4 align-middle text-muted-foreground ${col.key === "mentionCount" ? "text-right font-bold tabular-nums" : ""}`}>
                      {col.key === "canonicalName" ? (
                        <Link href={`/gold/models/${model.id}`} className="font-medium text-foreground hover:underline">
                          {model.canonicalName}
                        </Link>
                      ) : col.key === "avgSentiment" ? (
                        <div className={`inline-flex items-center justify-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${getSentimentVariant(model.avgSentiment)}`}>
                          {formatSentiment(model.avgSentiment)} / 10
                        </div>
