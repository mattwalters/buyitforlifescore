...
import ModelClientTable from "./client-table";
...
export default async function ModelsPage(props: {
  ...
  const models = await prisma.goldProduct.findMany({
    orderBy: {
      [sortBy]: orderDir,
    },
    take: pageSize,
  ...
  const total = await prisma.goldProduct.count();
  ...
  return (
    <div className="container mx-auto p-8 space-y-8">
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">
            View all {total.toLocaleString()} exact, canonical models.
          </p>
        </div>
        <a
          href="/api/export?type=goldModels"
      ...
      <ModelClientTable models={models} sortBy={sortBy} dir={dir} />
